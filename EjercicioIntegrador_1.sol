...
contract Mapping {
    ...
    function guardarActivoDoble(
        ...
        require(_usuario != address(0), "El address no puede ser 0x00");
        require(
            _activoId <= 999999 && _activoId >= 0,
            "Codigo de activo invalido"
        );
    ...
    function guardarActivoTriple(
        ...
        require(_usuario != address(0), "El address no puede ser 0x00");
        if (_activoId > 999999 || _activoId <= 0) revert("Codigo de activo invalido");

        if (_ciudadId > 999999 || _ciudadId <= 0) revert CiudadInvalidaError(_ciudadId);
