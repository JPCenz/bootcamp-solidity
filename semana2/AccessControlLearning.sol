...
contract AccessControlLearning {
    ...
    mapping(address account => mapping(bytes32 role => bool boolean)) _roles;
    ...
    mapping(address => mapping (bytes32 role => uint256)) _roleTemp;

    modifier onlyRole(bytes32 _role) {
        // require(hasTemporaryR(msg.sender,_role),"Cuenta no tiene el rol necesario");
        // _;
        require(hasRole(msg.sender,_role) || _hasTemporaryRole(msg.sender,_role), "Cuenta no tiene el rol necesario");
        _;
    }
    ...
    function hasRole(address _account, bytes32 role) public view returns (bool){
        bool srole = _roles[_account][role];
        return srole;
    }
    ...
    function grantRoleTemporarily(
        ...
        _roleTemp[_account][_role] = _limit;
    ...
    function hasTemporaryRole(
        ...
        uint256  limit = _roleTemp[_account][_role];
        if (limit < 1) {
            return (false, 0);
        }
        return (true,limit);
    ...
    function _hasTemporaryRole (address _account,bytes32 _role) private  returns(bool) {
        uint256 limit = _roleTemp[_account][_role];
        if(limit < 1) return false;
        if (_roleTemp[msg.sender][DEFAULT_ADMIN_ROLE] > 0){
            _roleTemp[msg.sender][DEFAULT_ADMIN_ROLE] -= 1 ;
        }
