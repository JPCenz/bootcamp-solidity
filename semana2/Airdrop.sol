...
contract Airdrop is AccessControl {
    ...
    mapping(address account => uint256 lastTimeAdded) _whiteList;
    mapping(address => uint256 lastTimeAdded) _blueList;
        ...
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        ...
        for (uint256 i = 0; i < _length; i++) {
            _whiteList[_addresses[i]] = _getCurrentTimestamp();
        }
    ...
    function mintWithWhiteList() external {
        ...
        require( _getCurrentTimestamp() - _whiteList[msg.sender] < 24 hours, "Pasaron mas de 24 horas");
    ...
    function burnMyTokensToParticipate() external {
        ...
        _whiteList[msg.sender] = _getCurrentTimestamp();
    }
        ...
        returns (uint256)
    {
        ...
        // r: tiempo restante para completar 60 minutos
        // m + r = 60 minutos
        //uint256 m = _getCurrentTimestamp() - _enterTime;
        uint256 totalTime = 60 * 60 ; // m + r -> 60 min x 60 sec
        uint256 timePased = _getCurrentTimestamp() - _enterTime; // m -> block.timestamp - _enterTime
    ...
    function _getRandom() internal view returns (uint256) {
        ...
        uint256 random = 0;
        random =  (uint256(keccak256(abi.encodePacked(msg.sender, address(this), block.timestamp)) )% 1000) +1;
